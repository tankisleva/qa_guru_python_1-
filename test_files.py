...
path_outputfiles = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'archives')  # путь к папке "archives"
...
def test_pdf():
    ...
    with zipfile.ZipFile(path_zip) as zf:
        pdf_file = zf.extract("fb.pdf")
        reader = PdfReader(pdf_file)
        page = reader.pages[0]
        text = page.extract_text()
        assert 'Facebook Ireland Ltd.' in text, f"Expected result: {'Facebook Ireland Ltd.'}; actual result: {text}"
        os.remove(pdf_file)
